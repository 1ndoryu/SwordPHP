...
if (env('CMS_ENABLED', true)) {
    ...
    $panelGroup = Route::group('/panel', function () {
        ...
        Route::group('/paginas', function () {
            Route::get('', [PaginaController::class, 'index']); Route::get('/create', [PaginaController::class, 'create']); Route::post('/store', [PaginaController::class, 'store']); Route::get('/edit/{id}', [PaginaController::class, 'edit']); Route::post('/update/{id}', [PaginaController::class, 'update']); Route::post('/destroy/{id}', [PaginaController::class, 'destroy']);
        });
        $tiposDeContenido = TipoContenidoService::getInstancia()->obtenerTodos();
        if (!empty($tiposDeContenido)) {
            $slugs = array_keys($tiposDeContenido); $slugs = array_filter($slugs, fn($slug) => $slug !== 'paginas');
            if (!empty($slugs)) {
                $slugRegex = implode('|', $slugs);
                Route::group('/{slug:' . $slugRegex . '}', function () {
                    Route::get('', [TipoContenidoController::class, 'index']); Route::get('/crear', [TipoContenidoController::class, 'create']); Route::post('/crear', [TipoContenidoController::class, 'store']); Route::get('/editar/{id:\d+}', [TipoContenidoController::class, 'edit']); Route::post('/editar/{id:\d+}', [TipoContenidoController::class, 'update']); Route::post('/eliminar/{id:\d+}', [TipoContenidoController::class, 'destroy']); Route::get('/ajustes', [TipoContenidoController::class, 'mostrarAjustes']); Route::post('/ajustes', [TipoContenidoController::class, 'guardarAjustes']);
                });
            }
        }
        Route::get('/temas', [TemaController::class, 'index']); Route::post('/temas/activar/{slug}', [TemaController::class, 'activar']);
        Route::group('/plugins', function () {
            Route::get('', [PluginController::class, 'index']); Route::post('/activar/{slug}', [PluginController::class, 'activar']); Route::post('/desactivar/{slug}', [PluginController::class, 'desactivar']);
        });
        Route::group('/ajustes', function () {
            Route::get('', [AjustesController::class, 'index']); Route::post('/guardar', [AjustesController::class, 'guardar']); Route::get('/enlaces-permanentes', [AjustesController::class, 'enlacesPermanentes']); Route::post('/enlaces-permanentes', [AjustesController::class, 'guardarEnlacesPermanentes']); Route::get('/{slug}', [PluginPageController::class, 'mostrar']);
        });
        Route::get('/media', [MediaController::class, 'index']); Route::post('/media/subir', [MediaController::class, 'subir']); Route::post('/media/destroy/{id}', [MediaController::class, 'destroy']);
        Route::group('/usuarios', function () {
            Route::get('', [UsuarioController::class, 'index']); Route::get('/crear', [UsuarioController::class, 'create']); Route::post('/crear', [UsuarioController::class, 'store']); Route::get('/editar/{id:\d+}', [UsuarioController::class, 'edit']); Route::post('/update/{id:\d+}', [UsuarioController::class, 'update']); Route::post('/eliminar/{id:\d+}', [UsuarioController::class, 'destroy']);
        });
    });
    $panelGroup->middleware([AutenticacionMiddleware::class]);

    // Rutas de Autenticación
    Route::get('/registro', [AuthController::class, 'mostrarFormularioRegistro']); Route::post('/registro', [AuthController::class, 'procesarRegistro']); Route::get('/login', [AuthController::class, 'mostrarFormularioLogin']); Route::post('/login', [AuthController::class, 'procesarLogin']); Route::get('/logout', [AuthController::class, 'procesarLogout']);
